...
export const POST = async (req: Request) => {
  ...
  if (user[0].isMember === false && studyMaterials.length >= 5) {
    return NextResponse.json(
      {
        error:
          "You have reached the limit of 5 study materials. Please upgrade to a premium plan to create more.",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    console.error("Error in generate-study-outline API:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to generate study outline",
      },
      { status: 500 }
    );
  }
